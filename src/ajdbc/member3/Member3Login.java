...
public class Member3Login extends JFrame implements ActionListener {
	...
	/***********************************************************************************
	 * 로그인 구현
	 * @param mem_id - 사용자가 입력한 id 받아오기
	 * @param mem_pw - 사용자가 입력한 pw 받아오기
	...
	public String login(String mem_id, String mem_pw) {
        ...
        sql.append(" FROM member                             ");
		  id가 pk가 아니기 때문에 중복이 허용되어 중복검사를 하고 난 이후에 회원가입버튼이 활성화되면 
		  중복된 id도 회원가입이 될 수 있다
		*/
		...
		try {
			...
			// System.out.println(rs.next()); // true - 커서가 다음으로 이동됨 아래 if문에서 false가 되어버림
			// 조회 결과는 0이거나 1row이므로 if문으로 처리
			if(rs.next()) { // false
