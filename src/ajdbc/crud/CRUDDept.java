...
public class CRUDDept extends JFrame implements ActionListener, MouseListener{
	...
	// 반복되는 테이블 패턴, 서로 의존관계에 있다. - 의존성 주입(인스턴스화-싱글톤패턴), 객체 주입법, annotion
	String		cols[] = {"부서번호","부서명","지역"};
	String		data[][] = new String[0][3]; // 데이터셋 객체배열
	...
	/******************************************************************
	 * 부서 등록 구현
	 * VO(Value Object) - 오라클 타입과 자바 타입비교 - 컬럼명과 VO전역변수와 Map의 키값은 반드시 일치해야 한다.
	 * @param pdVO - 사용자가 입력한 부서번호, 부서명, 지역을 받는다 - 복합데이터 클래스
	 * @return int - 1: 등록 성공, 0: 등록실패
	...
	public int deptInsert(DeptVO pdVO) {
        ...
        try {
			...
			// 오라클 서버에서 입력처리를 성공했을 때 1을 돌려 받는다.
			if(result == 1) { 
				deptSelectAll();
	...
	public List<Map<String, Object>> deptSelectAll(){
		...
		try {
			...
			// 자료구조 안에 데이터가 존재하는지 유무를 체크하는 메소드를 제공하는 인터페이스
			...
			while(iter.hasNext()) {
				...
				// 데이터셋인 DefaultTableModel에 조회결과 담기 - 반복처리함 => 10,20,30,40
				dtm.addRow(oneRow);
			}
		} catch (Exception e) {
			e.printStackTrace();
