...
// 클래스 분할
// 어떤 클래스이든지 main 메소드를 가질 수 있다.(자손이라도 main을 가진다)
public class B extends JDialog {
	// 생성자 안에서도 메소드를 호출할 수 있다.
	// 이 때는 메인메소드에서와는 다르게 인스턴스화 없이도 내 안에 있는 메소드이므로 호출이 가능하다.
	// [접근제한자] 클래스이름()
	public B() {
	...
	// 절차지향적인 코딩이 아니라 구조지향적인 코딩을 해야하는 자바 언어의 특징때문이다.
	public static void main(String[] args) {
		// 내 안에 있는 메소드라 하더라도 static이 있는 메인 메소드에서
		// non-static 메소드를 호출하려면 인스턴스화를 해야 한다.
		B b = new B();
		b.initDisplay();
	}
