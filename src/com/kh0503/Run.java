package com.kh0503;
// UML 도구를 이용해서 클래스 설계할 때 +:public, -:private, #:protected
class Circle {
	...
	public Circle() {} // 단 전역변수인 radius에 대해서는 초기화가 없다. 그러니까 0이다.
	public Circle(double radius) { // 파라미터가 있는 생성자가 하나라도 있으면 디폴트 생성자를 JVM이 제공안함
		this.radius = radius; // 생성자 호출시 파라미터로 넘어온 값이 (Call by value) 전역변수에 담긴다.
	}
	...
	public void setRadius(double radius) { // radius - 3.1이 복사된다. call by value
		this.radius = radius; // 파라미터로 복사된 3.1을 전역변수에 대입하세요
	}
