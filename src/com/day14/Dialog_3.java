...
public class Dialog_3 extends JDialog implements ActionListener{
   String title = null;
   boolean isView = false; // false이면 메모리에는 로딩이 되지만 화면에는 비활성화, true이면 화면을 보여준다.
   ...
   // 자손의 클래스를 인스턴스화 하면 화면을 그리는 initDisplay()가 호출되게 하자
   ...
   public void set(String title, boolean isView, Main_3 main_3) {
	   this.setTitle(title);
	   this.setVisible(isView);
	   // 파라미터로 넘어온 주소번지를 전역변수에 재저장하기 - 원본의 주소번지를 담는다.
	   this.main_3 = main_3;
   }
   @Override
