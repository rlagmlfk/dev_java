...
// 단 한 번만 인스턴스화한 자료를 가지고 모두 처리를 하고 싶다. 어떡하지?
// 입력할때는 빈칸을 제공하고 새로 입력한 값을 저장해야 한다. - 업무 역할
// 수정일때는 기존에 등록된 정보에 대해서 일부를 변경 지원한다.
// 입력한 내용 또는 수정할 때 기본정보를 제공하기 위해서라도 한 건에 대한 상세조회가 선행되어야 한다.
public class Main_3 extends JFrame implements ActionListener{
   ...
   public void refreshData() {
      ...
      // 새로고침 후에 자손 화면을 닫기(자손 클래스의 자원 반납을 받는 메소드 호출)
      dialog.dispose();
   }
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      if(btnInsert == obj) {
    	  // 자손 클래스에서 부모 클래스에 refreshData메소드를 호출하고 싶어요
    	  // 그래서 this를 자손클래스에 set메소드의 파라미터로 넘겨주자
