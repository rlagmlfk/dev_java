...
//컴넌트들의 위치를 정해줍니다.
	 ...
	 * @param editable 이른 인스턴스화 후 입력 컴포넌트에 쓰기를 허용할 건가? 아님 읽기전용으로 할 건가를 결정
	 * 		  		   입력(수정가능), 수정(수정가능), 상세보기(only read)
	 * @param vo
	 * @param abook
	 *********************************************************************************/
	...
	// 셋팅하는 메쏘드입니다.
	private void setEditable(boolean e) { // 파라미터 boolean, false
		txtName.setEditable(e); // true가 오면 이름을 변경가능함, false이면 변경 불가
  	...
  	private void btnOkayActionPerformed(ActionEvent evt) {
		...
		if(avo !=null){
			...
			try{
				...
				vo.setId(id);
				//JOptionPane.showMessageDialog(this, vo);
				// 입력이든 수정이든 반드시 AddressCtrl를 경유한다. - 컨벤션(MVC 패턴)
				AddressCtrl ctrl = new AddressCtrl();
				// vo안에는 전역변수가 있다. 읽고 (getter getXXX()) 쓰기(setter setXXX())
				ctrl.send(vo);
				if(vo.getResult()==1) {
					abook.refreshData();
				}
