...
public class AddressBook extends JFrame {
    ...
    private void initComponents() {
		...
		labelTimer = new JLabel("현재 시간 "+lt);
...
//				Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
	...
	private void addActionPerformed(ActionEvent evt) {
		...
		// AddressBook에서 입력버튼이 눌리면, abook에서 이벤트 감지
		// ModifyDialog에 set메소드를 호출
		// 호출할 때 파라미터로는 사용자가 선택한 버튼의 이름, 화면보여줄까?, AddressVO(왜냐면 새로 입력하는 거니까)
...
//				Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
				...
				vo.setCommand("select");
				vo.setId(id);
				AddressVO rVO = ctrl.send(vo);
				
				mDialog.set("수정", true, rVO, abook);
				mDialog.setVisible(true);
			} catch (Exception e) {
				JOptionPane.showMessageDialog(this, "데이터를 가져오는 중 발생했습니다."+e.toString(),"Error", JOptionPane.ERROR_MESSAGE);
			}
